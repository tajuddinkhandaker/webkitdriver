...
#if 0
    if (doc) {
        ResourceHandleManager::sharedInstance()->setCookie(url, value);
        return;
    } else {
        printf("Set strange cookie!\n");
    }
#endif
...
String cookies(const Document* /*document*/, const KURL& url)
{
    ...
    //    ResourceHandleManager::sharedInstance()->cookies(url).utf8().data());
    //return ResourceHandleManager::sharedInstance()->cookies(url).utf8().data();
