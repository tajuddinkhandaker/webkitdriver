...
bool WebKitDriver::initialize()
{
    ...
    if (tempDirName) {
        String tempDir = String::fromUTF8(tempDirName);
...
void WebKitDriver::cleanup()
{
    ...
    String path = WebCore::DatabaseTracker::tracker().databaseDirectoryPath();
    if (path.isNull())
        return;
    ...
    String cacheFile = WebCore::pathByAppendingComponent(WebCore::cacheStorage().cacheDirectory(), "ApplicationCache.db");
...
WebKitDriver::WebKitDriver(String userAgent) :
    m_textMagnifier(1.0),
    m_isEditable(false),
    ...
    pageClients.chromeClient = new WebCore::ChromeClientHl(this);
    pageClients.contextMenuClient = new WebCore::ContextMenuClientHl();
    pageClients.editorClient = editorClient;
    pageClients.dragClient = new WebCore::DragClientHl();
    pageClients.inspectorClient = new WebCore::InspectorClientHl();
    pageClients.geolocationClient = priv->geoClient;
